...
const getTree = async (prefill?: number) => {
...
test.skip("compare pg to sqlite(1000)", async (t) => {
...
test.serial("test entries returns correct values", async (t) => {
	const tree = await getTree(100)
...
test.serial("test bounds", async (t) => {
	const tree = await getTree(100)
...
test.skip("compare pg to sqlite(100) with interleaved deletes", async (t) => {
	const tree = await getTree()
	const sqliteTree = getSqliteTree()
